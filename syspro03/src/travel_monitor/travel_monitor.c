...
travel_monitor_t *travel_monitor_create(connection_t **sockets_array, unsigned int sockets_array_sz) {
    ...
    tm->sockets_array = sockets_array;
    tm->sockets_array_sz = sockets_array_sz;
    return tm;
}
...
void travel_monitor_travel_request(travel_monitor_t *tm, char *id, char *date, char *from
    ...
    if (bf_test(bloom, id) == 0) {
        ...
        for (int i = 0; i < tm->sockets_array_sz; i++) {
            read_msg(msg, SOCK_BUFFER_SZ, tm->sockets_array[i]->client_sock);
            if (!strcmp(msg, "NO") && isvaccinated == 0)
                strcpy(date_vaccinated, "NO");
            else if (!strcmp(msg, "YES")) { // if returned yes
                isvaccinated = 1;
                read_msg(msg, SOCK_BUFFER_SZ, tm->sockets_array[i]->client_sock); // get the date
...
void travel_monitor_search_vaccination_status(travel_monitor_t *tm, char *citizen_id) {
    //broadcast_signal(tm, SIGUSR2); // prepare monitors to expect a request
    //broadcast_string(tm, "REQUESTING_SEARCH_VACCINATION_STATUS"); // send request type
    broadcast_int(tm, REQUESTING_SEARCH_VACCINATION_STATUS);
    broadcast_string(tm, citizen_id); // send the citizen id 
    char line[150];
    for (int i = 0; i < tm->sockets_array_sz; i++) { // for each monitor
        read_msg(line, SOCK_BUFFER_SZ, tm->sockets_array[i]->client_sock); // get a YES or NO
        if (!strcmp(line, "NO")) // if returned NO, then continue with the next monitor
            continue;
        else if (!strcmp(line, "YES")) { // if returned YES
            for (;;) {
                read_msg(line, SOCK_BUFFER_SZ, tm->sockets_array[i]->client_sock);
...
void travel_monitor_update_blooms(travel_monitor_t *tm, char *countryname, connection_t *con) {
    ...
    char virusname[50];
    write_msg(countryname, strlen(countryname)+1, SOCK_BUFFER_SZ, con->client_sock); // send the country name
    for (;;) {
        // read virus
        read_msg(virusname, SOCK_BUFFER_SZ, con->client_sock);
        if (!strcmp(virusname, "STOP UPDATING BLOOMS"))
            break;
        // read bloom filter
        read_msg((byte_t *) bitarr, SOCK_BUFFER_SZ, con->client_sock);
...
void broadcast_string(travel_monitor_t *tm, char *msg) { // broadcasts to all open monitors the msg string
    for (int i = 0; i < tm->sockets_array_sz; i++) {
        write_msg((byte_t *)msg, strlen(msg) + 1, SOCK_BUFFER_SZ, tm->sockets_array[i]->client_sock);
    }
}

void broadcast_int(travel_monitor_t *tm, int integer) {
    for (int i = 0; i < tm->sockets_array_sz; i++) {
        write_msg((byte_t *)&integer, sizeof(int), SOCK_BUFFER_SZ, tm->sockets_array[i]->client_sock);
    }
}
